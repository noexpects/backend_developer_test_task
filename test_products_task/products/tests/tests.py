...
class ProductDetailViewTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user = UserFactory()
        cls.url = reverse('products:product_detail', args=(cls.category.slug, cls.product.slug ))
...
class LikeToggleTestCase(TestCase):
    ...
    def test_anonymous_like_product(self):
        ...
        self.assertTrue(Like.objects.filter(product=self.product_1))
        self.assertTrue(Like.objects.filter(product=self.product_1).first().ip)

    def test_anonymous_dislike_product(self):
        self.client.post(self.url_like)
        self.client.post(self.url_like)
        self.assertFalse(Like.objects.filter(product=self.product_1))

    def test_user_like_product(self):
        self.client.login(username=self.user.username, password=self.password)
        self.client.post(self.url_like)
        self.assertTrue(Like.objects.filter(product=self.product_1))
        self.assertTrue(Like.objects.filter(product=self.product_1).first().user)
